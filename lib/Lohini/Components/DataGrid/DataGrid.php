...
implements \ArrayAccess
{
	 ...
	 * @return \ArrayIterator
	 */
	public function getColumns($type='Lohini\Components\DataGrid\Columns\IColumn')
	{
	 ...
	 * @return ArrayIterator
	 */
	public function getFilters($type='Lohini\Components\DataGrid\Filters\IColumnFilter')
	{
		$filters=new \ArrayObject;
		foreach ($this->getColumns() as $column) {
			if ($column->hasFilter() && ($filter=$column->getFilter()) instanceof $type) {
	 ...
	 * @return ArrayIterator
	 */
	public function getActions($type='Lohini\Components\DataGrid\IAction')
	{
		$actions=new \ArrayObject;
		foreach ($this->getColumns('Lohini\Components\DataGrid\Columns\ActionColumn') as $column) {
			if ($column->hasAction()) {
	...
	{
		if ($value<0) {
			throw new \Nette\InvalidArgumentException("Parameter must be non-negative number, '$value' given.");
			}
	...
	/**
	 * Applycators caller - filters data grid items
	 */
	...
	protected function applyItems()
	{
		...
		$this->itemsPerPage= $this->paginator->itemsPerPage= $value==0
	...
	public function render()
	{
		if (!$this->wasRendered) {
			...
			if (!$this->hasColumns() || count($this->getColumns('Lohini\Components\DataGrid\Columns\ActionColumn'))==count($this->getColumns())) {
		...
		echo mb_convert_encoding(call_user_func_array([$this->getRenderer(), 'render'], $args), 'HTML-ENTITIES', 'UTF-8');
	}
	...
	protected function createTemplate($class=NULL)
	{
		...
		if (NULL!==($trans=$this->getTranslator())) {
			$template->setTranslator($trans);
			}
		return $template;
	}
	...
	protected function createComponentForm($name)
	{
		...
		$form->setAction(new \Nette\Application\UI\Link(
			$this->presenter,
			$form->lookupPath('Nette\Application\UI\Presenter').self::NAME_SEPARATOR.'submit!',
			[]
			));
		...
		$form->addSubmit('operationSubmit', 'Perform')
	 ...
	 * @param string $func function used to format
	 * @return Columns\DateColumn
	 */
	public function addDateColumn($name, $caption=NULL, $format='%x', $func='strftime')
	{
		return $this[$name]=new Columns\DateColumn($caption, $format, $func);
	}
	 ...
	 * @param bool $useAjax use ajax? (add class DataGridColumn::$ajaxClass into generated link)
	 ...
	 * 				   otherwise variable Lohini\Components\DataGrid\DataGrid::$keyName will be used and must be defined)
	 * @return Action
	 * @throws \Nette\InvalidStateException
	 */
	public function addAction($title, $signal, $icon=NULL, $useAjax=FALSE, $key=Action::WITH_KEY)
	{
		if (!$this->hasColumns('Lohini\Components\DataGrid\Columns\ActionColumn')) {
			throw new \Nette\InvalidStateException('No Lohini\Components\DataGrid\Columns\ActionColumn defined. Use Lohini\Components\DataGrid\DataGrid::addActionColumn before you add actions.');
			}
	...
	public function translate($s)
	{
		return $this->translator===NULL
				? $s
				: call_user_func_array([$this->getTranslator(), 'translate'], func_get_args());
	}
	...
	{
		return $this->getSession()->getSection('Lohini.DataGrid/'.$this->getName().'/states');
	}
	...
	{
		return mb_convert_encoding(call_user_func_array([$this->getRenderer(), 'render'], [$this]), 'HTML-ENTITIES', 'UTF-8');
	}
}
