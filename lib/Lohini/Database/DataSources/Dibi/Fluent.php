...
extends \Lohini\Database\DataSources\Mapped
{
		...
		if (is_array($operation)) {
			...
			foreach ($operation as $t) {
				...
				if ($t===self::IS_NULL || $t===self::IS_NOT_NULL) {
					...
					$conds[]=['%n', $this->mapping[$column], $t, "%$modifier", $value];
					}
				}
	...
	public function getFilterItems($column)
	{
		/** @var \DibiFluent */
		$df=clone $this->df;
		return $df->removeClause('select')->select()->distinct($column)->limit(NULL)->fetchPairs($column, $column);
	}
